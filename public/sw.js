...
const network = (r) =>
  fetch(r).then((d) =>
    d.ok && r.method.toLowerCase() === "get"
      ? caches.open(assetCacheName).then((c) =>
          c
            .delete(r)
            .then(() => c.put(r, d.clone()))
            .then((_) => d)
        )
      : d
  );
...
const checkCache = (req) =>
  caches.open(assetCacheName).then((c) => c.match(req));
...
self.addEventListener("fetch", (e) => {
  ...
  const maybeCachedRequest = (
    ...
    .catch(
      (ce) => (
        console.log("[Service Worker] Request failed", e.request),
        console.error(ce)
      )
    )
    .then((resp) =>
      resp && resp.url.endsWith(".js")
        ? resp.text().then((rtext) => [rtext, resp])
        : resp
    )
    .then((r) =>
      Array.isArray(r)
        ? new Response(
            r[0]
              .replaceAll(
                /(from|import) ['"]three['"]/g,
                "$1 'https://cdn.jsdelivr.net/npm/three@0.169.0/build/three.webgpu.js'"
              )
              .replaceAll(
                /(from|import) ['"]three\/nodes['"]/g,
                "$1 'https://cdn.jsdelivr.net/npm/three@0.169.0/src/nodes/Nodes.js'"
              ),
            {
              headers: r[1].headers,
            }

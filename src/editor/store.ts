...
import { compare, mapStore} from "../nodysseus";
import { Edge, EdgesIn, Graph, GraphNode, isNodeRef, NodysseusNode, NodysseusStore, NodysseusStoreTypes, RefNode, RefStore, Store, ValueNode } from "../types";
...
export const automergeRefStore = async ({nodysseusidb, persist = false} : {persist: boolean, nodysseusidb: IDBPDatabase<NodysseusStoreTypes>}): Promise<RefStore> => {
  ...
  const refs: RefStore = {
      get: (id) => generic_nodes[id] ?? structuredCloneMap.get(id) 
...
export const sharedWorkerRefStore = async (): Promise<RefStore> => {
  ...
  sharedWorker.port.addEventListener('message', (e: MessageEvent<NodysseusStoreResponseMessage>) =>
    (console.log("received", e), e).data.kind === "connect" ? connectres()
    : inflightRequests.get(e.data.id)(e.data))
  console.log(sharedWorker)
  sharedWorker.port.start();

  
  setTimeout(() => (console.log("posting message"), sharedWorker.port.postMessage({kind: "keys", id: performance.now().toFixed()})), 1000);
  ...
  return {
      get: graphid => {
        ...
        return messagePromise({kind: "get", graphid}).then(e => (e as { kind: "get", id: string, graph: Graph }).graph)
      },

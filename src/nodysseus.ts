...
const node_nodes = (
  ...
  node_id: string,
  data,
  graph_input_value: Env,
  lib: Lib,
  options: RunOptions,
) => {
  return wrapPromise(
    typeof node === "string" ? nolib.no.runtime.get_ref(node) : node,
  ).then((graph) =>
    run_graph(
      graph,
      node_id,
      combineEnv(data, graph_input_value, node_id, graph_input_value._output),
      lib,
      options,
    ),
  ).value;
...
const node_script = (
  ...
  // const data = {};
  const data = resolve_args(
    nodeArgs,
    lib,
    options.resolvePromises ? options : { ...options, resolvePromises: true },
  );
    ...
    .then((promisedData) =>
      lib.data.no.of(
        fn.apply(null, [
          ...
          data,
          ...Object.values(
            isError(promisedData) ? promisedData : promisedData?.value ?? {},
          ),
        ]),
      ),
    ).value;
...
export const run_extern = (
  ...
  const args =
    typeof extern === "function"
      ? resolve_args(data, lib, options)
      : externArgs.map(([arg]) => {
          let newval;
          if (arg === "_node") {
            newval = node;
            ...
            newval = extern.rawArgs
              ? data
              : wrapPromise(resolve_args(data, lib, options))
                  .then((data) => (isError(data) ? data : data.value))
                  .then((args) => new Map(Object.entries(args))).value;
          } else if (arg == "_lib") {
            newval = lib;
  ...
  if (argspromise && !extern.promiseArgs) {
    ...
    const resArgs = Array.isArray(args)
      ? args
      : isValue(args)
        ? args.value.args
        : args;
...
const resolve_args = (
  ...
  for (const k of data.keys()) {
    ...
    const resarg = (argval) =>
      wrapPromise(isValue(argval) ? argval.value : argval).then((runnable) =>
        isConstRunnable(runnable)
          ? resarg(run_runnable(runnable, lib, undefined, options))
          : runnable,
      ).value;
    val = resarg(val);
  ...
  if (is_promise && options.resolvePromises) {
    ...
    Object.entries(result).forEach((kv) => {
      promises.push(
        Promise.resolve(kv[1]).then((pv: Result) =>
          isError(pv) ? pv : [kv[0], isValue(pv) ? pv?.value : pv],
        ),
      );
    });
    return Promise.all(promises)
      .then(Object.fromEntries)
      .then((v) => lib.data.no.of(v));
  }
...
const node_data = (nodeArgs, graphArgs: Env, lib, options) => {
  return nodeArgs.size === 0 || graphArgs._output === "display"
    ? lib.data.no.of(undefined)
    : resolve_args(nodeArgs, lib, options);
};

const createFunctorRunnable = (
  fn: Exclude<Runnable, Result | ApRunnable>,
  parameters: ConstRunnable,
  lib,
  options: RunOptions,
  output: ConstRunnable,
  includeFullGraph: boolean = false,
): FunctorRunnable | Promise<FunctorRunnable> =>
  fn &&
  wrapPromiseAll([
  ...
  ]).then(([args, refs, output]: [Result, Array<string>, string]) =>
    isError(args)
      ? args
      : lib.data.no.of({
          __kind: FUNCTOR,
          parameters: args
            ? [
                ...new Set(
                  args.value
                    ? Object.keys(args.value).map((k) =>
                        k.includes(".") ? k.substring(0, k.indexOf(".")) : k,
                      )
                    : [],
                ),
              ]
...
const createGraphFunctorRunnable = (
  graph: ConstRunnable,
  parameters: ConstRunnable,
  lib: Lib,
  options: RunOptions,
  output: ConstRunnable,
  graphid: ConstRunnable,
  env: ConstRunnable,
  out: ConstRunnable,
): FunctorRunnable | Promise<FunctorRunnable> =>
  graph &&
  ...
  ]).then<FunctorRunnable>(
    ([params, resultGraph, resultOutput, graphid, env, out]: [
      Result,
      Result,
      Result,
      Result,
                              ...
                              newEnv(
                                new Map([
                                  [
                                    ...
                                    (graphid &&
                                      !isError(graphid) &&
                                      lib.data.no.of(graphid.value)) ||
                                      lib.data.no.of(resultGraph.value),
                                  ],
                                ]),
                                resultOutput.value,
                              ),
...
const run_runnable = (
  ...
  switch (runnable.__kind) {
    case CONST:
      return wrapPromise(getRunnableGraph(runnable, lib)).then(
        (graph) =>
          wrapPromise(
            run_graph(
              graph,
              runnable.fn,
              mergeEnv(args, runnable.env),
              runnable.lib,
              options,
            ),
            (e) =>
              handleError(
                ...
                runnable.graph,
                runnable.fn,
                (runnable.env.data.get("__graphid") as NonErrorResult).value,
              ),
          ).value,
      ).value;
    case AP:
...
const run_node = (
  node: { node: NodysseusNode; graph?: Graph } | Runnable,
  nodeArgs: Map<string, ConstRunnable>,
  graphArgs: Env,
  lib: Lib,
  options: RunOptions,
): Result | Promise<Result> => {
  if (isRunnable(node)) {
    if (isError(node)) {
      ...
      const graphid = (graphArgs.data.get("__graphid") as { value: string })
        .value;
      const nodegraphargs: Env =
        node.env ?? newEnv(new Map(), graphArgs._output);
      nodegraphargs.data.set(
        "__graphid",
        graphid ?? lib.data.no.of(node.graph),
      );
    ...
    if (node.node.ref === "arg") {
      if (graphArgs._output === "metadata") {
        ...
        const edgeChain = [];
        const descGraph = descendantGraph(
          node.node.id,
          node.graph,
          (nodeId, edge) => {
            edgeChain.push(edge);
            ...
            const dataNode =
              ...
              edge.as !== "args"
                ? node.graph.nodes[
                    Object.values(node.graph.edges_in[nodeId]).find(
                      (e) => e.as === "args",
                    )?.from
                  ]
                : nodeId &&
            ...
            if (dataNode) {
              if (
                (node.graph.edges_in[dataNode.id] &&
                  ...
                  run(
                    {
                      ...
                      lib,
                      args: newEnv(
                        new Map([["output", lib.data.no.of("metadata")]]),
                      ),
                    },
                    graphArgs,
                  ),
                ).then((r) => ({ nodeId: e[0], inEdge: e[1], metadata: r }))
                  .value,
            ),
        )
          .then((descgraphmetas) =>
                  ...
                  .reduce(
                    (acc, e) =>
                      !acc || Array.isArray(acc)
                        ? acc
                        : acc.type === "@flow.runnable"
                          ? acc.runnableParameters
                          : typeof acc.type === "object"
                            ? acc.type[e.as]
                            : acc,
                    dgm.metadata?.parameters?.[dgm.inEdge[0].as],
                  ),
              )
              .flat()
              .filter((v) => v),
          )
          .then((v) => lib.data.no.of({ values: keys.concat(v) })).value;
      }
      const resval = nolib.no.arg(
        node.node,
        graphArgs,
        lib,
        node.node.value,
      ...
      return wrapPromise(resval).then((resval) =>
        resval && typeof resval === "object" && isValue(resval)
          ? resval
          : lib.data.no.of(resval),
      ).value;
    ...
    const newGraphArgs = newEnv(
      new Map().set("__graphid", lib.data.no.of(newgraphid)),
      graphArgs._output,
    );

    return wrapPromise(lib.data.no.runtime.get_ref(node.node.ref), (e) =>
      handleError(e, lib, graphArgs, graphid, node),
    ).then((node_ref) => {
      if (!node_ref) {
        throw new Error(
          `Unable to find ref ${(node.node as RefNode).ref} for node ${
            node.node.name || node.node.id
          }`,
        );
      }
      ...
      lib.data.no.runtime.set_parent(
        newgraphid,
        graphid,
        (node.node as RefNode).ref,
      );

      const result = run_node(
        { node: node_ref },
        (node.node as RefNode).value
          ? new Map(nodeArgs).set(
              "__graph_value",
              lib.data.no.of((node.node as RefNode).value),
            )
          : nodeArgs,
        newGraphArgs,
        lib,
    ...
    node.node.edges_in =
      node.node.edges_in ??
      Object.values(node.node.edges).reduce<Graph["edges_in"]>(
        (acc, e) => ({ ...acc, [e.to]: { ...(acc[e.to] ?? {}), [e.from]: e } }),
        {},
      );
    ...
    const env =
      isError(graphid) || isConstRunnable(graphid)
        ? graphArgs
        : combineEnv(
            new Map([["__graphid", lib.data.no.of(`${graphid.value}`)]]),
            graphArgs,
            undefined,
            graphArgs._output,
          );
    ...
    return (
      (graphArgs._output === undefined || graphArgs._output === "value") &&
      lib.data.no.of(node_value(node.node))
    );
  } else {
    ...
    return nodeArgs.size === 1 && nodeArgs.keys().next().value.startsWith("arg")
      ...
      : (graphArgs._output === undefined ||
          graphArgs._output === "value" ||
          graphArgs._output === "display") &&
          node_data(nodeArgs, graphArgs, lib, {
            ...options,
...
const create_data = (
  ...
  lib: Lib,
  options: RunOptions,
): Map<string, ConstRunnable> | Promise<Map<string, ConstRunnable>> => {
  return wrapPromiseAll([
    nolib.no.runtime.get_edges_in(graph, node_id),
    nolib.no.runtime.refs(),
    ...
    const newgraphargs = graphArgs._output
      ? mergeEnv(new Map().set("_output", undefined), graphArgs)
      : graphArgs;
          ...
          if (as_set.has(e.as)) {
            throw new NodysseusError(
              nodysseus_get(
                ...
                "/" +
                node_id,
              `Multiple input edges have the same label "${e.as}"`,
            );
          }
          as_set.add(e.as);
        });
...
const run_graph = (
  ...
  try {
    return wrapPromise(node)
      .then(
        (node) =>
          wrapPromise(create_data(node_id, newgraph, env, lib, options)).then(
            (data) => ({ node, data }),
          ).value,
      )
      .then(({ node, data }) => {
        ...
        if (
          options.profile &&
          !nolib.no.runtime.get_parentest(
            (env.data.get("__graphid") as { value: string }).value,
          )
        ) {
          const edgePath = (edge) => {
            const edgeout = nolib.no.runtime.get_edge_out(newgraph, edge);
            return edgeout && !ispromise(edgeout)
              ? [nolib.no.runtime.get_edge_out(newgraph, edge).as].concat(
                  edgePath(nolib.no.runtime.get_edge_out(newgraph, edge).to),
                )
              : ["promise"];
          };
          ...
          const result = run_node(
            isRunnable(node) ? node : { node, graph },
            data,
            env,
            lib,
            options,
          ...
          if (!ispromise(result) && options?.profile && id) {
            ...
            options.timings[id] =
              (options.timings[id] ?? 0) + (performance.now() - start);
          }

          return isResPromise && options.profile && id
            ? result.then((v) => {
                // console.timeEnd(id)
                // performance.mark(`${id} - end`);
                // performance.measure(id, `${id} - begin`, `${id} - end`);
...
const run_functor_runnable = (
  runnable: FunctorRunnable,
  args: Args,
  lib: Lib,
  options: RunOptions,
): Result | Promise<Result> => {
  const execArgs: Args = new Map(
    runnable.parameters?.map((k) => [k, nodysseus_get(args, k, lib)]) ?? [],
  );
  const newRunnable: ConstRunnable = {
    __kind: CONST,
    env: combineEnv(
      (execArgs ?? new Map()).set(
        "__graphid",
        runnable.env.data.get("__graphid"),
      ),
      runnable.env,
      runnable.fn,
      runnable.env._output,
    ),
    fn: runnable.fn,
...
const run_ap_runnable = (
  ...
  return wrapPromise(
    isRunnable(runnable.args)
      ? run_runnable(runnable.args, lib, args, {
          ...options,
    ...
    const execArgs = isValue(ranArgs)
      ? ranArgs.value instanceof Map
        ? ranArgs.value
        : new Map(
            [...Object.entries(ranArgs.value)]
              .filter((kv) => kv[0] !== "__graphid")
              .map((kv) => [kv[0], lib.data.no.of(kv[1])]),
          )

...
import { isNodeGraph, isValue, isNodeRef, isApRunnable, isConstRunnable, isRunnable, isNodeScript, isEnv, isArgs, isError, FUNCTOR, CONST, AP, isApFunction, isApFunctorLike, getRunnableGraph, isLib, isFunctorRunnable, isGraph, isInputRunnable } from "./types.js";
...
export const mapStore = () => {
    ...
    return {
        get: id => map.get(id),
        set: (id, data) => (map.set(id, data), data),
...
export const nodysseus_get = (obj, propsArg, lib, defaultValue = undefined, props = [], options = {}) => {
    let objArg = obj;
...
const run_extern = (extern, data, lib, options, node, graphArgs) => {
    ...
    const externArgs = isArgsArray ? extern.args.map(a => {
    ...
    if (argspromise && !extern.promiseArgs) {
        return (Array.isArray(args) ? Promise.all(args) : args.then(v => isValue(v) ? v.value.args : v)).then(as => {
...
const node_extern = (node, data, graphArgs, lib, options) => {
...
const resolve_args = (data, lib, options) => {
    ...
    if (is_promise && options.resolvePromises) {
        ...
        Object.entries(result).forEach(kv => {
            promises.push(Promise.resolve(kv[1]).then((pv) => isError(pv) ? pv : [kv[0], isValue(pv) ? pv?.value : pv]));
        });
...
const createFunctorRunnable = (fn, parameters, lib, options, output, includeFullGraph = false) => fn && wrapPromiseAll([
    ...
    .then(([args, refs, output]) => isError(args) ? args : lib.data.no.of({
    __kind: FUNCTOR,
    ...
    graph: includeFullGraph && typeof fn.graph === "string"
...
const createGraphFunctorRunnable = (graph, parameters, lib, options, output, graphid) => graph && wrapPromiseAll([
    parameters && run_runnable(parameters, lib, undefined, options),
    graph && run_runnable(graph, lib, undefined, options),
    output && run_runnable(output, lib, undefined, options),
]).then(([params, resultGraph, resultOutput]) => isError(params)
    ? params
...
const run_runnable = (runnable, lib, args = new Map(), options = {}) => {
    ...
    switch (runnable.__kind) {
        case CONST:
            return wrapPromise(getRunnableGraph(runnable, lib))
                .then(graph => wrapPromise(run_graph(graph, runnable.fn, mergeEnv(args, runnable.env), runnable.lib, options), e => handleError(e, lib, runnable.env, runnable.graph, runnable.fn)).value).value;
        case AP:
            return run_ap_runnable(runnable, args, lib, options);
        case FUNCTOR:
            return wrapPromise(run_functor_runnable(runnable, args, lib, options), e => handleError(e, lib, runnable.env, runnable.graph, runnable.fn)).value;
    }
    return runnable;
};
// graph, node, symtable, parent symtable, lib
const run_node = (node, nodeArgs, graphArgs, lib, options) => {
    if (isRunnable(node)) {
        if (isError(node)) {
            return node;
        }
            ...
            const nodegraphargs = node.env ?? newEnv(new Map(), graphArgs._output);
        ...
        node.edges_in = node.edges_in ?? Object.values(node.edges).reduce((acc, e) => ({ ...acc, [e.to]: { ...(acc[e.to] ?? {}), [e.from]: e } }), {});
...
const create_data = (node_id, graph, graphArgs, lib, options) => {
...
const handleError = (e, lib, env, graph, node) => {
    ...
    lib.data.no.runtime.publish("grapherror", new NodysseusError(nodysseus_get(env, "__graphid", lib)?.value + "/" + error_node.id, e instanceof AggregateError ? "Error in node chain" : e));
    return e;
};
// handles graph things like edges
const run_graph = (graph, node_id, env, lib, options) => {
    ...
    try {
        return wrapPromise(node).then(node => wrapPromise(create_data(node_id, newgraph, env, lib, options)).then(data => ({ node, data })).value)
            ...
            if (options.profile && !nolib.no.runtime.get_parentest(env.data.get("__graphid").value)) {
                ...
                const id = `${path} - ${env.data.get("__graphid").value}/${node.id} (${node.ref})`;
...
const run_functor_runnable = (runnable, args, lib, options) => {
    const execArgs = new Map(runnable.parameters?.map(k => [k, nodysseus_get(args, k, lib)]) ?? []);
    const newRunnable = {
        __kind: CONST,
...
const run_ap_runnable = (runnable, args, lib, options) => {
        ...
        const ret = (Array.isArray(runnable.fn) ? runnable.fn : [runnable.fn])
            ...
            return typeof rfn === "function"
                ...
                : isApFunction(rfn)
                    ...
                    : run_runnable(rfn, runnable.lib, execArgs, options);
        });
...
export const run = (node, args = new Map(), options = {}) => {
    ...
    return wrapPromise(getRunnableGraph(node, _lib)).then(nodeGraph => {
        ...
        for (let k of args.keys()) {
            if (!isValue(args.get(k))) {
                args.set(k, nolib.no.of(args.get(k)));
            }
        }
        const res = run_runnable(isRunnable(node)
...
const runtimefn = () => {
    ...
    const change_graph = (graph, lib, changedNodes = [], broadcast = false, source) => wrapPromise(get_parentest(graph))
        .then(parent => {
        if (parent) {
    ...
    const remove_ref = (id) => wrapPromise(nodysseus.refs.keys()).then(keys => {
        if (keys.includes(id)) {
    ...
    const get_node = (graph, id) => graph.nodes[id];
    const get_edge = (graph, from) => wrapPromise(get_graph(graph)).then(g => g?.edges[from]).value;
    const get_edges_in = (graph, id) => Object.hasOwn(graph.edges_in, id) ? Object.values(graph.edges_in[id]) : [];
    ...
    const get_graph = (graph) => wrapPromise(nodysseus.refs.get(typeof graph === "string" ? graph : graph.id))
        .then((g) => {
    ...
    return {
        ...
        addRefsFromUrl: (url) => nodysseus.refs.addFromUrl(url),
        remove_ref,
        get_node,
        get_edge,
        get_edges_in,
        ...
        get_fn: (id, name, orderedargs, script) => {
            ...
            return wrapPromise(nodysseus.fns.get(fnid + orderedargs)).then(fn => {
                if (!fn || fn.script !== script) {
                    ...
                    fn = Object.assign(fn ?? {}, {
                        script,
                        fn: new Function(`return function _${name.replace(/(\s|\/)/g, "_")}(_lib, _node, _graph_input_value${orderedargs}){${script}}`)(),
        ...
        graphExport: (graphid) => Object.keys(generic.nodes).includes(graphid) ? []
            : wrapPromise(get_graph(graphid))
                .then(graph => wrapPromiseAll(Object.values(graph.nodes)
                .map(node => isNodeRef(node) ? nolib.no.runtime.graphExport(node.ref) : []))
                .then(nodeGraphs => nodeGraphs.flat().concat([graph])).value).value,
        change_graph,
        update_graph: (graphid, lib) => publish('graphupdate', { graph: graphid }, lib),
        update_args,
        ...
        updateGraph: async ({ graph, addedNodes = [], addedEdges = [], removedNodes = [], removedEdges = [], lib, dryRun = false }) => {
            ...
            // }
            return change_graph(nodysseus.refs.get(graphId), lib, addedEdges.flatMap(e => [e.from, e.to]));
        },
        add_node: (graph, node, lib) => {
            ...
            change_graph(nodysseus.refs.get(graphId), lib, [node.id]);
        },
        add_nodes_edges: (graph, nodes, edges, remove_edges, remove_nodes, lib) => {
            ...
            change_graph(nodysseus.refs.get(graphId), lib, nodes.concat(remove_nodes)
                .map(n => n.id)
                .concat(edges.concat(remove_edges).flatMap(e => [e.to, e.from])));
        },
        delete_node: (graph, id, lib, changeEdges = true) => {
            wrapPromise(get_graph(graph)).then(graph => {
                ...
                const new_child_edges = child_edges.map((e, i) => ({
                    ...
                    as: i === 0
                        ...
                        : !e.as
                            ? e.as
                            : current_child_edges.find((ce) => ce.as === e.as && ce.from !== id)
                                ? e.as + "1"
                                : e.as,
                }));
                if (changeEdges === undefined) {
                    ...
                    change_graph(nodysseus.refs.get(graphId), lib, current_child_edges.concat(new_child_edges).flatMap(e => [e.to, e.from]));
                }
            });
        },
        addListener,
        ...
        undo: (id) => nodysseus.refs.undo && nodysseus.refs.undo(id),
        redo: (id) => nodysseus.refs.redo && nodysseus.refs.redo(id),
        store: nodysseus,
        ancestor_graph
    };
};
const nolib = {
    no: {
        of: (value) => ispromise(value) ? value.then(nolib.no.of) : isValue(value) ? value : { __kind: "result", value: value },
        arg: (node, target, lib, value, options) => {
            ...
            // }
            let resolveret = (rr) => wrapPromise(rr)
    ...
    extern: {
        ...
        ap: {
            ...
            fn: (fn, args, run, lib) => {
                ...
                const apRunnable = (fnRunnable, run) => ({
                    __kind: AP,
        ...
        switch: {
            ...
            fn: (input, args, node_args, lib, options, otherwise, removeSubscriptions, graphid) => {
                return wrapPromiseAll([
                    ...
                    if (removeSubscriptions && lib.data.no.runtime.get_args(graphid).value !== ival) {
                        removeSubscriptions && (isArgs(args) ? [...args.entries()] : Object.entries(args))
                            ...
                            .flatMap((runnableEntry) => Object.values(ancestor_graph(runnableEntry[1].fn, lib.data.no.runtime.get_ref(runnableEntry[1].graph), lib).nodes).map(n => [runnableEntry[0], runnableEntry[1].graph, n]))
                            .forEach(([inputid, graph, node]) => lib.data.no.runtime.pauseGraphListeners(`${graph.id}/${node.id}`, ival !== inputid));
                        lib.data.no.runtime.update_args(graphid, { value: ival });
                    }
                    return isError(ival)
                        ? ival
                        : run_runnable(isArgs(args)
                            ? args.get(ival)
                            : args[ival], lib, undefined, options)
        ...
        map: {
            ...
            fn: (fn, array, lib, options) => wrapPromise(run_runnable(array, lib, undefined, options))
                ...
                ? wrapPromise(run_runnable(fn, lib, undefined, options))
                    ...
                    ? wrapPromiseAll(arr.map((element, index) => typeof fnr === "function" ? fnr(mergeEnv(new Map([["element", lib.data.no.of(element)], ["index", lib.data.no.of(index)]]), fn.env))
                        : run_runnable(fnr, lib, new Map([["element", lib.data.no.of(element)], ["index", lib.data.no.of(index)]]), options)).map(v => wrapPromise(v).then(v => isValue(v) ? v.value : v))).then(vs => lib.data.no.of(vs))
                    : isError(fnr) ? fnr
                        : arr)
                : arr).value
        },
        fold: {
            ...
            fn: (fn, object, initial, lib, options) => wrapPromise(run_runnable(object, lib, undefined, options))
                .then(ov => isError(ov) ? ov : ov.value)
                .then(objectvalue => objectvalue === undefined ? undefined
                : isError(objectvalue) ? objectvalue
                    : wrapPromise(run_runnable(fn, lib, undefined, options))
                        .then(fnr => isError(fnr) ? fnr : fnr.value)
                        .then(fnrunnable => wrapPromise(run_runnable(initial, lib, undefined, options))
                        .then(initial => isError(initial) ? initial : initial.value)
                        .then(initial => {
                        const mapobjarr = (mapobj, mapfn, mapinit) => typeof mapobj.reduce === "function"
                        ...
                        const ret = mapobjarr(objectvalue, (previousValue, currentValue) => !errored && wrapPromise(previousValue).then(prevVal => {
                            const args = new Map().set("previousValue", lib.data.no.of(prevVal)).set("currentValue", lib.data.no.of(currentValue));
                            return typeof fnrunnable === "function" ? fnrunnable(args) : run_runnable(fnrunnable, lib, args, options);
                        }).then(rv => {
                            if (isError(rv)) {
                                errored = true;
                                return rv;
        ...
        state: {
            ...
            fn: (value, lib, graphid, options, output, persist, publish) => {
                ...
                return wrapPromise(publish && run_runnable(publish, lib, undefined, options))
                    ...
                    .then(publish => wrapPromise(persist && run_runnable(persist, lib, undefined, options))
                    ...
                    .then(({ persist, publish, initial }) => wrapPromise(rawstate)
        ...
        return: {
            ...
            args: {
                ...
                "metadata": {
                    type: {
                        parameters: (graph, nodeId) => ({ type: Object.fromEntries(Object.values(ancestor_graph(nodeId, graph).nodes).filter(n => isNodeRef(n) && n.ref === "arg" && n.value && !n.value.startsWith("_")).map((n) => [n.value.includes('.') ? n.value.split('.')[0] : n.value, "any"])) }),
                        values: "any",
                        dataLabel: "any"
                    },
                },
            ...
            fn: ({ value, display, subscribe, metadata, args, lib, _output, _lib, _runoptions, __graphid }) => {
                ...
                const return_result = (_lib, args) => {
                    ...
                    if (edgemap.subscribe && (runedge === "display" || runedge === "value")) {
                        ...
                        wrapPromise(run_runnable(edgemap.subscribe, _lib, args, { ...options, resolvePromises: true }))
                            ...
                            && _lib.data.no.runtime.addListener(kv[0], newgraphid, kv[1], false, graphid, true, _lib, options)));
                    }
        ...
        get: {
            ...
            fn: ({ target, path, def, __graph_value, _lib, _output }) => {
                const getPrototypeNames = (obj) => obj && typeof obj === "object" && Object.getPrototypeOf(obj) ? Object.getOwnPropertyNames(obj)
                ...
                return _path ? _lib.data.no.nodysseus_get(_path.startsWith("lib") ? _lib.data : target, _path.startsWith("lib") ? _path.substring(3) : _path, _lib, def) : target;
            },
        },
        set: {
            args: ["target: default", "path", "value", "__graph_value"],
            fn: (target, path, value, nodevalue) => {
                const ispromise = (a) => a && typeof a.then === 'function' && !isWrappedPromise(a);
                ...
                const check = (o, v, k) => k.length === 1
                    ...
                    : o && typeof o === "object" && Object.hasOwn(o, k[0])
                        ...
                        : o;
                const ret = (((value !== undefined && ispromise(value)) || ispromise(target)
                    ? Promise.all([
        ...
        script: {
            ...
            fn: (node, node_inputs, graph, _lib, options, _output) => _output === "metadata" ? {
                dataLabel: "script"
            } :
                node_script(node, node_inputs, _lib, options),
        },
        new_array: {
            args: ["_node_args", "__graph_value"],
            fn: (args, nodevalue) => nodevalue ? nodevalue.split(/,\s+/)
                : wrapPromiseAll(Object.entries(args).sort((akv, bkv) => akv[0].localeCompare(bkv[0])).map((kv) => wrapPromise(kv[1]))).then(r => r.map(rv => isValue(rv) ? rv.value : rv)).value
        },
        fetch: {
            resolve: true,
        ...
        call: {
            ...
            fn: ({ __graph_value, self, fn, args, _lib, _output }) => {
                const getPrototypeChainMethods = (obj) => obj && typeof obj === "object" && Object.getPrototypeOf(obj) ? Object.getOwnPropertyNames(obj)
        ...
        merge_objects_mutable: {
            ...
            fn: (target, args, lib, options) => {
                const merge = (target = {}, value) => Object.entries(value)
        ...
        construct: {
            ...
            fn: ({ args, name, __graph_value, _lib }) => {
                ...
                return fn && typeof fn === "function" && new (Function.prototype.bind.apply(fn, [null, ...(args === undefined ? [] : Array.isArray(args) ? args : [args])]));
            }
        },
        addEventListeners: {
            ...
            fn: (target, nodeargs, lib) => {
                Object.entries(nodeargs)
                    ...
                    .forEach(([k, fn]) => target[k] = event => fn && run_runnable(fn, lib, new Map().set("event", event)));
                return target;
            }
        },
        data: {
        ...
        graphState: {
            args: ["graphid"],
            fn: (graphid) => wrapPromise(nodysseus.persist.keys())
                .then(keys => wrapPromiseAll(keys

...
import { decodeSyncState, encodeSyncState } from "@automerge/automerge";
import { isNodeGraph, isNodeRef, isNodeValue } from "src/types.js";
...
export const automergeRefStore = async ({ nodysseusidb, persist = false, graphChangeCallback }) => {
    ...
    const structuredCloneMap = new Map();
    const refsset = new Set(await nodysseusidb.getAllKeys("refs").then(ks => ks.map(k => k.toString()).filter(k => k)));
    const syncedSet = new Set();
    /////
    // Fns
    //
    const createDoc = () => Automerge.applyChanges(Automerge.init(), [initgraph])[0];
    const updatePeers = async (id, target) => {
        if (id === "custom_editor")
            return;
        syncedSet.add(id);
        ...
        updatePeersDebounces[id] = setTimeout(() => {
            wrapPromise(refsmap.get(id)).then(current => {
                ...
                (target ? [[target, syncStates[target]]] : Object.entries(syncStates)).forEach(([peer, syncState]) => {
                    const [nextSyncState, syncMessage] = Automerge.generateSyncMessage(current, syncState[id] || Automerge.initSyncState());
    ...
    const getDoc = (id) => refsmap.has(id) ? refsmap.get(id)
        : refsset.has(id)
            ? nodysseusidb.get("refs", id)
                .then(persisted => {
                let doc = Automerge.load(persisted);
                refsmap.set(id, doc);
    ...
    const changeDoc = (id, fn, changedNodes = []) => {
        ...
        return wrapPromise(getDoc(id))
            .then(graph => {
            let doc = Automerge.change(graph ?? createDoc(), { patchCallback: graphNodePatchCallback }, fn);
            persist && nodysseusidb.put("refs", Automerge.save(doc), id);
            refsmap.set(id, doc);
            refsset.add(id);
            updatePeers(id);
    ...
    const nodeCompare = (a, b) => a.id === b.id && a.name === b.name &&
    ...
    const graphCompare = (a, b) => a.id === b.id
    ...
    const refs = {
        addFromUrl: url => fetch(url)
            .then(res => res.json())
            .then(refsToAdd => Promise.all(refsToAdd.map((ref) => refs.set(ref.id, ref))).then(gs => gs)),
        ...
        set: (id, graph) => wrapPromise(refs.get(id))
        ...
        add_nodes_edges: ({ graphId, addedNodes, addedEdges, removedEdges, removedNodes }) => changeDoc(graphId, graph => {
    ...
    syncBroadcast.addEventListener("message", v => {
        ...
        if (data.type === "syncstart" && syncStates[data.peerId]?._syncType !== "broadcast" && data.peerId !== peerId) {
            ...
            wrapPromise(getDoc(data.id)).then(currentDoc => {
                ...
                const [nextDoc, nextSyncState, patch] = Automerge.receiveSyncMessage(currentDoc, syncStates[data.peerId]?.[id] || Automerge.initSyncState(), data.syncMessage, {
    ...
    setTimeout(() => {
        wrapPromise(getDoc("custom_editor"))
            ...
            syncWS.addEventListener("message", (ev) => {
                    ...
                    if (messageType === "syncstart" || messageType === "syncgraph") {
                        ...
                        if (data.type === "syncstart" && data.peerId !== peerId) {
                            ...
                            wrapPromise(getDoc(id)).then(current => {
                                ...
                                const [nextDoc, nextSyncState, patch] = Automerge.receiveSyncMessage(current, syncStates[data.peerId]?.[id] || Automerge.initSyncState(), data.syncMessage, {
                                    patchCallback: graphNodePatchCallback
                                });
                                persist && nodysseusidb.put("refs", Automerge.save(nextDoc), id);
                                refsset.add(id);
                                refsmap.set(id, nextDoc);
                                const scd = structuredClone(nextDoc);
                                scd.edges_in = {};
                                Object.values(scd.edges).forEach(edge => {
                                    if (scd.edges_in[edge.to]) {
                                        scd.edges_in[edge.to][edge.from] = { ...edge };

...
import { hashcode } from "../../nodysseus.js";
import { isNodeGraph } from "../../types.js";
import { ispromise } from "../../util.js";
import { isd3NodeNode } from "../types.js";
import { calculateLevels, CreateNode, hlib, pzobj, SelectNode, graphEdgesIn, setRootNodeXNodeY } from "../util.js";
export const UpdateSimulation = (dispatch, payload) => payload ? !(payload.simulation || payload.static) ? undefined : updateSimulationNodes(dispatch, payload) : dispatch(state => [state, [() => !(state.simulation) ? undefined : updateSimulationNodes(dispatch, state), undefined]]);
export const UpdateGraphDisplay = (dispatch, payload) => {
...
export const getLinks = (simulation) => simulation ? simulation.simulation.force('links').links()
...
export const getNodes = (simulation) => simulation ? simulation.simulation.nodes().filter(isd3NodeNode) : [];
export const updateSimulationNodes = (dispatch, data) => {
    ...
    if (!data.clear_simulation_cache) {
        data.simulation.simulation.force('links').links()
            .forEach(l => {
    ...
    const parentsMapEntries = Object.values(data.editingGraph.nodes)
        .map(n => [n.id, graphEdgesIn(data.editingGraph, n.id).map(e => e.from)]);
    const hasPromise = !parentsMapEntries.every(e => !ispromise(e[1]));
    // Just skip if there's a pending promise
    if (hasPromise)
        return;
    const parents_map = new Map(parentsMapEntries);
    ...
    const nodes = order.flatMap(nid => {
        ...
        const calculated_nodes = !child ? [{
                ...
                x: Math.floor(simulation_node_data.get(node_id)?.x
                    ?? simulation_node_data.get(parents_map.get(n.id)?.[0])?.x
                    ?? addorundefined(simulation_node_data.get(child)?.x, sibling_mult * (256 + 32 * Math.log(Math.max(1, ancestor_count.get(n.id)) / Math.log(1.01))))
                    ?? Math.floor(window.innerWidth * (randpos.x * .5 + .25))),
                y: Math.floor(simulation_node_data.get(node_id)?.y
                    ?? addorundefined(256, simulation_node_data.get(parents_map.get(n.id)?.[0])?.y)
                    ?? addorundefined(-(16 + 32 * Math.log(Math.max(1, ancestor_count.get(n.id) * 0.5 + parents_map.get(children_map.get(n.id))?.length)) / Math.log(1.25)), simulation_node_data.get(children_map.get(n.id))?.y)
                    ?? Math.floor(window.innerHeight * (randpos.y * .5 + .25)))
            }];
        if (child) {
    ...
    const links = Object.values(data.editingGraph.edges)
        ...
        const proximal = ((parents_map.get(e.to)?.length ?? 0) +
            (parents_map.get(children_map.get(e.to))?.length ?? 0)) * 0.5;
        return {
            ...e,
            edge: e,
            source: e.from,
    ...
    const linkNameNodes = links.map(l => ({
        ...l,
    ...
    if (typeof (links?.[0]?.source) === "string") {
        if (simulation_node_data.size !== start_sim_node_size ||
            ...
            data.simulation.simulation.force('links')?.links().length !== links.length) {
            data.simulation.simulation.alpha(0.8);
        }
        data.simulation.simulation.nodes(nodes.concat(linkNameNodes));
        data.simulation.simulation.force('links').links(links);
    ...
    data.simulation.simulation.force('link_direction')
        .y((n) => (((parents_map.get(n.node_id)?.length > 0 ? 1 : 0)
        ...
        * (logmaxparents + 3) + .5) * window.innerHeight)
        .strength(n => isd3NodeNode(n)
        ? (!!parents_map.get(n.node_id)?.length === !children_map.has(n.node_id))
            || children_map.get(n.node_id)?.length > 0 ? .01 : 0
        : 0.01);
    data.simulation.simulation.force('link_label_x')
        .x(n => 0)
        .strength(n => isd3NodeNode(n) ? 0 : 0.01);
    data.simulation.simulation.force('collide').radius(n => isd3NodeNode(n) ? 96 : 8);
...
const idFromNode = (node) => typeof node === "object" ? node.node_id : typeof node === "number" ? node.toFixed() : node;
        ...
        if (simulation.alpha() > simulation.alphaMin()) {
            ...
            simulation.force('links').links().map(l => {
                ...
                if (el && edge_label_el) {
                    ...
                    linkNameNode.desiredX = edgeLabelX;
                    linkNameNode.desiredY = edgeLabelY;
                    if (insert_el) {
...
const node_text_el = ({ node_id, primary, focus_primary, secondary, primaryClass, edgeName }) => ha.h('text', { x: 48, y: 12 }, [
...
export const node_el = ({ html_id, selected, error, selected_distance, node_id, node_ref, node_name, node_value, has_nodes, nested_edge_count, nested_node_count, node_parents, edgeName }) => ha.h('g', {
    ...
    ha.h('circle', { class: { shape: true, none: true, error }, r: radius * 0.5, cx: radius * 0.5 + 8, cy: radius * 0.5 + 12 }),

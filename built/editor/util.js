...
import { initStore, nodysseus_get, nolib, run, nolibLib } from "../nodysseus.js";
import { getRunnableGraphId, isArgs, isNodeGraph, isNodeRef, isNodeScript, isTypedArg } from "../types.js";
...
import { isd3NodeNode } from "./types.js";
...
export const pzobj = {
    ...
    effect: function (dispatch, payload) {
        requestAnimationFrame(() => {
            ...
            const viewbox = findViewBox(nodes, getLinks(payload.simulation), payload.node_id, payload.node_el_width, payload.html_id, payload.dimensions);
    ...
    init: function (dispatch, sub_payload) {
        ...
        let init = requestAnimationFrame(() => {
            ...
            hlib.panzoom.instance.on('transform', (e) => {
                document.getElementById("node-editor-editor").style.setProperty("--zoom-scale", hlib.panzoom.instance.getTransform().scale.toString());
            });
        });
...
export const isNodysseusError = (e) => e && (e instanceof nolib.no.NodysseusError || e.cause?.node_id);
...
export const setRootNodeXNodeY = ({ position, simulation }) => {
    if (pzobj.centered) {
...
export const SetSelectedPositionStyleEffect = (_, payload) => {
...
export const CreateNode = (state, { node, child, child_as, parent }) => [
    state,
    dispatch => {
        if (!node.id)
            node.id = create_randid(state.editingGraph);
...
export const Paste = state => [
    ...
    [dispatch => {
            navigator.clipboard.readText().then(clipboardContents => {
                ...
                const node_id_map = {};
                nolib.no.runtime.add_nodes_edges(state.editingGraph, Object.values(copied.graph.nodes)
                    .map(n => ({ ...n, id: (node_id_map[n.id] = create_randid(state.editingGraph), node_id_map[n.id]) })), Object.values(copied.graph.edges)
                    .map(e => ({ ...e, from: node_id_map[e.from], to: node_id_map[e.to] }))
                    .concat([{ from: node_id_map[copied.root], to: state.selected[0], as: copied.as }]), [], [], hlibLib);
...
export const SelectNode = (state, { node_id, focus_property, clearInitialLayout }) => state.editingGraph.nodes[node_id] ? [
...
export const FocusEffect = (_, { selector }) => {
    setTimeout(() => {
...
export const UpdateNodeEffect = (dispatch, { editingGraph, node }) => {
...
export const UpdateNode = (state, { node, property, value, editingGraph }) => [
    state,
    [UpdateNodeEffect, {
            editingGraph: editingGraph ?? state.editingGraph,
            node: Object.fromEntries(Object.entries(Object.assign({}, base_node(typeof node === "string" ? (editingGraph ?? state.editingGraph).nodes[node] : node ? node : state.editingGraph.nodes[state.selected[0]]), { [property]: value === "" ? undefined : value })).filter(kv => kv[1] !== undefined))
        }]
];
export const UpdateEdge = (state, { edge, as }) => [
    state,
    [() => {
            nolib.no.runtime.updateGraph({
                graph: state.editingGraph,
...
export const result_subscription = (dispatch, { editingGraphId, displayGraphId, norun }) => {
    ...
    return () => (nolib.no.runtime.removeListener('graphupdate', 'clear_hyperapp_error'),
    ...
    const listener = ({ graph }) => {
        ...
        if (props.editingGraphId === graph.id) {
            ...
            animframe = requestAnimationFrame(() => {
                animframe = false;
                dispatch((s) => [{ ...s, editingGraph: graph }, UpdateSimulation, [refresh_graph, {
            ...
            animframe = requestAnimationFrame(() => {
                animframe = false;
                dispatch((s) => [s, UpdateSimulation, [refresh_graph, {
...
export const listenToEvent = (dispatch, props) => {
...
export const node_args = (nolib, graph, node_id, cachedMetadata = {}) => {
    ...
    return wrapPromise(cachedMetadata[node_id] ?? hlib.run(graph, node_id, { _output: "metadata" }))
        ...
        const node_out_args = node_out?.ref === "@flow.runnable" ?
            ...
            : nodeOutNodeArg?.type && typeof nodeOutNodeArg.type === "object"
                ? Object.entries(nodeOutNodeArg.type).map(e => typeof e[1] === "function" ? [e[0], e[1](graph, edge_out.to)] : e)
                : undefined;
        // const argslist_path = node_ref?.nodes && nolib.no.runtime.get_path(node_ref, "argslist");
        const nextIndexedArg = "arg" + ((edges_in?.filter(l => l.as?.startsWith("arg") && new RegExp("[0-9]+").test(l.as.substring(3)))
        ...
        const baseargs = externfn
            ...
            : isNodeGraph(node_ref)
                ? Object.values(node_ref?.nodes)
                    .filter(isNodeRef)
                    .filter(n => n.ref === "arg")
                    .map(n => parseTypedArg(n.value))
                    .filter(a => typeof a[1] === "string" || !a[1].local) ?? []
                : [];
        const scriptVarNames = node.ref === "@js.script" && new Set();
        if (node.ref === "@js.script" && typeof node.value === "string") {
            const scriptVarDecs = node.ref === "@js.script" && new Set();
            parser.parse(node.value).iterate({
                enter: syntaxNode => syntaxNode.name === "VariableName" && !window[node.value.substring(syntaxNode.from, syntaxNode.to)]
                    ...
                    : syntaxNode.name === "VariableDeclaration" && syntaxNode.node.getChild("VariableDefinition")
                        ? !!scriptVarDecs.add(node.value.substring(syntaxNode.node.getChild("VariableDefinition").from, syntaxNode.node.getChild("VariableDefinition").to))
                        : undefined
            });
        ...
        const typedArgsMap = new Map(edges_in?.map(e => [e.as, { type: "any", additionalArg: !baseargs.map(a => a[0]).includes(e.as) }])
            .concat(baseargs)
            .filter((a) => !a[0].includes('.') && !a[0].startsWith("_"))
            .concat((externArgs && externArgs.map(a => a[0]).includes("_node_args") || baseargs.map(a => a[0]).includes("_args"))
...
export const graphEdgeOut = (graph, node) => graph.edges[node];
export const graphEdgesIn = (graph, node) => graph["edges_in"]
...
export const hlibLib = mergeLib(nolibLib, newLib({
    ...
    worker: undefined,
    workerPostMessage: (runnable, args, transferrableObjects) => {

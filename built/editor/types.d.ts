...
import { Graph, Edge, NodysseusNode, EdgeNoAs, NodeMetadata, NodeArg } from "../types.js";
export type Vector2 = {
    x: number;
    y: number;
};
...
export declare const isd3NodeNode: (n: d3Node) => n is d3NodeNode;
...
export type TRespondableSharedWorkerMessage<T> = T extends _SharedWorkerMessageKind ? _SharedWorkerMessages[T] extends {
    to: any;
    from: any;
} ? TSharedWorkerMessageTo<T> | TSharedWorkerMessageFrom<T> : never : never;
...
type _SharedWorkerMessageId<T> = T extends _SharedWorkerMessageKind ? _SharedWorkerMessages[T] extends {
    to: any;
    from: any;
} ? {
    messageId: string;
} : {} : never;
type _SharedWorkerMessage<T> = T extends _SharedWorkerMessageKind ? _SharedWorkerMessages[T] : never;
...
type _TSharedWorkerMessageTo<T> = T extends _SharedWorkerMessageKind ? _SharedWorkerMessage<T> extends {
    to: any;
} ? _SharedWorkerMessage<T>["to"] & {
    kind: T;
} : never : never;
export type TSharedWorkerMessageToData<T> = _TSharedWorkerMessageTo<T>;
export type TSharedWorkerMessageTo<T> = T extends _SharedWorkerMessageKind ? _SharedWorkerMessages[T] extends {
    to: any;
    from: any;
} ? _TSharedWorkerMessageTo<T> & {
    messageId: string;
} : _TSharedWorkerMessageTo<T> : never;
export type SharedWorkerMessageTo = TSharedWorkerMessageTo<_SharedWorkerMessageKind>;
type _TSharedWorkerMessageFrom<T> = T extends _SharedWorkerMessageKind ? _SharedWorkerMessage<T> extends {
    from: any;
} ? _SharedWorkerMessage<T>["from"] & {
    kind: T;
} : never : never;
export type TSharedWorkerMessageFrom<T> = T extends _SharedWorkerMessageKind ? _SharedWorkerMessages[T] extends {
    to: any;
    from: any;
} ? _TSharedWorkerMessageFrom<T> & {
    messageId: string;
} : _TSharedWorkerMessageFrom<T> : never;
export type SharedWorkerMessageFrom = TSharedWorkerMessageFrom<_SharedWorkerMessageKind>;
export declare const expectSharedWorkerMessageResponse: <T extends keyof _SharedWorkerMessages>(m: SharedWorkerMessageTo | SharedWorkerMessageFrom) => m is TRespondableSharedWorkerMessage<T>;
export {};

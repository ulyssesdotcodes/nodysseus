import { isNodeGraph, isNodeValue, isEnv, isRunnable, isValue, isArgs, isConstRunnable, isEdgesInGraph } from "./types.js";
export const WRAPPED_KIND = "wrapped";
export const ispromise = (a) => a && typeof a.then === 'function' && !isWrappedPromise(a);
export const isWrappedPromise = (a) => a && a.__kind === WRAPPED_KIND;
...
export const wrapPromise = (t, c) => (isWrappedPromise(t) ? t
    : {
        __kind: WRAPPED_KIND,
        then: (fn) => wrapPromise(ispromise(t)
            ? c ? t.then(fn).catch(c)
                : t.then(fn)
            : tryCatch(fn, t, c)),
        value: t
    });
export const wrapPromiseAll = (wrappedPromises) => {
...
export const create_randid = (graph) => {
...
const isFlattenedGraph = (g) => !!g.flat_nodes;
export const flattenNode = (graph, levels = -1) => !isNodeGraph(graph) || !graph.nodes || levels <= 0
    ? graph
    : Object.values(graph.nodes)
        .map(g => flattenNode(g, levels - 1))
        .reduce((acc, n) => isFlattenedGraph(n) ? Object.assign({}, acc, {
...
export const expand_node = (data) => {
    ...
    const node = data.editingGraph.nodes[node_id];
    if (!isNodeGraph(node)) {
    ...
    const new_id_map = isFlattenedGraph(flattened) ? Object.values(flattened.flat_nodes).reduce((acc, n) => nolib.no.runtime.get_node(data.editingGraph, n.id) ? (acc[n.id] = create_randid(data.editingGraph), acc) : n, {}) : flattened;
    isFlattenedGraph(flattened) && nolib.no.runtime.add_nodes_edges(data.editingGraph.id, Object.values(flattened.flat_nodes).map(n => ({ ...n, id: new_id_map[n.id] ?? n.id })), Object.values(flattened.flat_edges).concat(in_edges.map((e) => ({ ...e, to: new_id_map[args_node] ?? args_node }))).concat([{ ...data.editingGraph.edges[node_id], from: node.out }]).map(e => ({ ...e, from: new_id_map[e.from] ?? e.from, to: new_id_map[e.to] ?? e.to })), in_edges.concat([data.editingGraph.edges[node_id]]), [node_id], nolib);
    ...
    if (!isNodeGraph(node)) {
        ...
        while (q.length > 0) {
            ...
            if (!args_edge || e.from !== args_edge.from) {
                nolib.no.runtime.get_edges_in(data.editingGraph, e.from).forEach((de) => q.push(de));
            }
        }
...
export const descendantGraph = (nodeId, graph, nolib) => {
    const outGraph = { ...graph, nodes: {} };
    while (nodeId) {
 ...
 * Type utils
 */
export const newEnv = (data, _output, env) => ({ __kind: "env", data: data?.size > 0 ? env?.data?.size ? new Map([...env.data, ...data]) : data : new Map(), _output, env: env?.env });
export const combineEnv = (data, env, node_id, _output) => {
    if (isEnv(data)) {
...
export const newLib = (data) => ({ __kind: "lib", data });
export const mergeLib = (a, b) => (a ? {
    __kind: "lib",
...
const mergeDeep = (a, b) => {
    ...
    return aKeys.concat(bKeys).reduce((acc, key) => acc[key]
        ? acc
        : { ...acc, [key]: MERGE_KEYS.includes(key) && typeof a[key] === "object" && typeof b[key] === "object"
                ? mergeDeep(a[key], b[key])
                : a[key] ?? b[key]
        }, {});
};
export const runnableId = (runnable) => isConstRunnable(runnable) ? `${runnable.graph}/${runnable.fn}` : false;

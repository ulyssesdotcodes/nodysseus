import { isApFunction, isApRunnable, isError, isFunctorRunnable, isValue } from "./types.js";
...
export const initListeners = () => {
    ...
    const runpublish = (data, event, lib, options = {}, broadcast = true) => {
        ...
        const listenerMap = getorset(event_listeners, event, () => new Map());
        ...
        const runlistener = (l) => {
            try {
                if (typeof l === "function") {
                    ...
                    run(l, Object.assign({}, isApRunnable(l) ? l.args : {}, { data }), { ...options, lib: mergeLib(l.lib, lib) });
                }
            }
            catch (e) {
                console.error(e);
        ...
        if (!pause || event === "graphchange" || event === "grapherror") {
            for (let l of listeners) {
                if (!l[0].startsWith("__system") && !(l[1].graphid && [...pausedGraphIds.keys()].find(k => l[1].graphid.startsWith(k))))
                    runlistener(l[1].fn);
                // runlistener(l)
            }
        }
    ...
    const publish = (event, data, lib, options = {}, broadcast = true) => {
    ...
    const addListener = (event, listener_id, input_fn, remove = false, graph_id = false, prevent_initial_trigger = false, lib = { __kind: "lib", data: nolib }, options = {}) => {
        ...
        const replaceGraphs = (runnable) => isApFunction(runnable) || typeof runnable === "function" || isValue(runnable) || isError(runnable) || runnable === undefined || (isFunctorRunnable(runnable) && typeof runnable.graph === "object")
            ? runnable
        ...
        const fn = typeof input_fn === "function"
            ? input_fn
            : (args) => {
    ...
    const systemgraphchangelistener = ({ graph, dirtyNodes }) => {
    ...
    const pauseGraphListeners = (graph_id, paused) => paused ? pausedGraphIds.add(graph_id) : pausedGraphIds.delete(graph_id);
    const isGraphidListened = (graphId) => event_listeners_by_graph.has(graphId);
    const isListened = (event, listenerId) => event_listeners.get(event)?.has(listenerId);
    return { publish, addListener, pauseGraphListeners, removeListener, isGraphidListened, isListened, togglePause: (newPause) => pause = newPause };
};
